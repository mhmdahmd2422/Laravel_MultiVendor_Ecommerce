    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query){
                $deleteButton = "<a href='".route('vendor.products-image-gallery.destroy', $query->id)."' style='color: white;' class='btn btn-danger ml-1 delete-item'><i class='fas fa-trash-alt mr-1'></i></a>";
                return $deleteButton;
            })            ->addColumn('image', function ($query){
                return $image = "<img style='height: 7rem; width: 9rem;' src='".asset($query->image)."'>";
            })
