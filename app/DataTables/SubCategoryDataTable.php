    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('parent', function ($query){
                $icon = "<span>{$query->category->name}</span>";
                return $icon;
            })
            ...
            ->addColumn('action', function ($query){
                $editButton = "<a href='".route('admin.sub-category.edit', $query->id)."' style='color: white;' class='btn-sm btn-warning'><i class='far fa-edit mr-1'></i></a>";
                $deleteButton = "<a href='".route('admin.sub-category.destroy', $query->id)."' style='color: white;' class='btn-sm btn-danger ml-1 delete-item'><i class='fas fa-trash-alt mr-1'></i></a>";
                return $editButton.$deleteButton;
            })
            ->rawColumns(['parent', 'action', 'status'])
            ->setRowId('id');
    }
